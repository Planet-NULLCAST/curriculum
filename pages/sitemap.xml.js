  ...
  try {
    ...
    for (let i = 0; i < courses.length; i++) {
      ...
      for (let j = 0; j < courses[i].chapters.length; j++) {
        chapters.push(
          `${CLIENT_URL}/curriculum/${courses[i].courseUrl}/${courses[i].chapters[j].chapterUrl}`
        );
      }
      const siteMapCoursesData = chapters.map((cou) => {
        return `
      <url>
        <loc>${cou}</loc>
        <lastmod>${getDate}</lastmod>
      ...
      fs.writeFileSync(
        `${dirname}/public/sitemap-${courses[i].courseUrl}.xml`,
        formattedSitemap,
        "utf8"
      );
    ...
    const siteMapCoursesList = course.map((couLis) => {
      return `
      <url>
    ...
    fs.writeFileSync(
      `${dirname}/public/sitemap-courses.xml`,
      formattedSitemap,
      "utf8"
    );
